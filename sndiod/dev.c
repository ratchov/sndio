...
mtc_midi_full(void)
{
	...
	mtc.qfr = 0;
	dev_midi_send(mmc_dev, (unsigned char *)&x, SYSEX_SIZE(full));
}
...
ctl_log(struct ctl *c)
{
	...
	case CTL_SLOT_OPT:
		log_puts("slot_opt:");
		log_puts(c->u.slot_opt.slot->name);
		log_putu(c->u.slot_opt.slot->unit);
		log_puts("/");
		log_puts(c->u.slot_opt.opt->name);
		break;
	case CTL_OPT_DEV:
		log_puts("opt_dev:");
